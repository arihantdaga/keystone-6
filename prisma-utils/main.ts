...
// MongoDB does not support Decimal type
const MONGODB_SCALARS = ['String', 'Boolean', 'Int', 'Float', 'DateTime', 'BigInt'] as const
...
model Optional {
  ...
  ${MONGODB_SCALARS.map(scalarType => `${scalarType} ${scalarType}?`).join('\n  ')}
}
...
async function generate(provider: Provider) {
  ...
  return [
    ...
    // case sensitivity is supported for PostgreSQL and MongoDB

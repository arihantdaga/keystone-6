...
function printField(
  ...
  if (field.kind === 'relation') {
    ...
    const map = field.foreignIdField.map ? ` @map(${JSON.stringify(field.foreignIdField.map)})` : ''
    const relationIdField = `${relationIdFieldPath} ${foreignIdField.scalar}?${map} ${nativeType}${index}`
...
export function printPrismaSchema(config: KeystoneConfig, lists: Record<string, InitialisedList>) {
  const { prismaClientPath, provider, extendPrismaSchema: extendPrismaCompleteSchema } = config.db

  const datasourceBlock = [
    `datasource ${provider} {`,
    `  url = env("DATABASE_URL")`,
    ...(provider !== 'mongodb' ? [`  shadowDatabaseUrl = env("SHADOW_DATABASE_URL")`] : []),
    `  provider = "${provider}"`,
    `}`,
  ]
  ...
  for (const [
    ...
    for (const [fieldPath, field] of Object.entries(resolvedDbFields)) {
      ...
      if (field.kind !== 'none') {
        let fieldPrisma = printField(fieldPath, field, provider, lists, provider)
        if (fieldPath === 'id') {
          fieldPrisma += ' @id'
        }
